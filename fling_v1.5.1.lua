...
if getgenv().FLING_ENABLED then
    if getgenv().FLING_LISTENER then
        getgenv().FLING_LISTENER:Disconnect()
    end
    
    getgenv().FLING_LISTENER = uis.InputBegan:Connect(function(key, paused)
        if not getgenv().FLING_ENABLED then
            getgenv().FLING_LISTENER:Disconnect()
            return
        end
